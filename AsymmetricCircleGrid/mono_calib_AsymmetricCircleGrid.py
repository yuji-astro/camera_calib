...
def asymmetric_world_points(chessboard_size):
        pattern_points = []
        pattern_rows = chessboard_size[0]
        pattern_columns = chessboard_size[1]
        if pattern_rows < pattern_columns:
        ...
        pattern_points = np.hstack((pattern_points,np.zeros((pattern_rows*pattern_columns,1)))).astype(np.float32)
...
# チェスボードの物理的な座標
objp = asymmetric_world_points(chessboard_size) #np.zeros((np.prod(chessboard_size), 3), np.float32)

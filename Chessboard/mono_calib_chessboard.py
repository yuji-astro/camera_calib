...
if flag_init_cam_param:
    ret, camera_matrix, dist_coeffs, rvecs, tvecs, std_devs_rvecs, std_devs_tvecs,per_view_error = cv2.calibrateCameraExtended(obj_points, img_points, gray.shape[::-1], init_camera_matrix, None, [],[],flags=cv2.CALIB_USE_INTRINSIC_GUESS)
else:
    ret, camera_matrix, dist_coeffs, rvecs, tvecs, std_devs_rvecs, std_devs_tvecs,per_view_error = cv2.calibrateCameraExtended(obj_points, img_points, gray.shape[::-1], None, None)
...
camera_params = {
    ...
    'std_devs_rvecs': [std_devs_rvec.tolist() for std_devs_rvec in std_devs_rvecs],
    'std_devs_tvecs': [std_devs_tvec.tolist() for std_devs_tvec in std_devs_tvecs],
    'per_view_error': per_view_error.tolist(),
}
...
print(f"Rotation Vectors std:\n{std_devs_rvecs}")
print(f"Translation Vectors std:\n{std_devs_tvecs}")
...
for i, fname in enumerate(images):
    if detected_results[i][1]:  # コーナーが検出された画像のみ処理
        ...
        for p1, p2 in zip(img_points1, img_points2):
            p1 = tuple(map(float, p1.ravel()))
            p2 = tuple(map(float, p2.ravel()))
            x_len = int(100*(p2[0] - p1[0]))
            y_len = int(100*(p2[1] - p1[1]))
            # 矢印を描画 (スケール50倍)
            arrow_p2 = (int(p1[0]) + x_len, int(p1[1]) + y_len)
            cv2.arrowedLine(img, tuple(map(int,p1)), arrow_p2, (0, 255, 0), 2)  # 矢印は緑
...
with open(log_file_path, 'w') as f:
    ...
    f.write("Image File, Corners Detected, RMSE, rvec,,,tvec,,,rvec std,tvec_std\n")
    # 各画像の結果を記載
    for i, (fname, detected) in enumerate(detected_results):
        if detected:
            ...
            std_devs_rvec = std_devs_rvecs[detected_img_idx].flatten()
            std_devs_tvec = std_devs_tvecs[detected_img_idx].flatten()
            f.write(f"{fname}, {detected}, {rmse:.5f}, {rvec.tolist()}, {tvec.tolist()}, {std_devs_rvec.tolist()}, {std_devs_tvec.tolist()}\n")
            detected_img_idx += 1
        else:
            f.write(f"{fname}, {detected}, N/A, N/A,,,N/A,,,N/A,,,N/A\n")
